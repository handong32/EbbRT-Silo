    ...
    const char *filename, int lineno)
{
  ...
  cerr << func << " (" << filename << ":" << lineno << ") - Condition `"
       << condstr << "' failed!" << endl;
  t.dump_debug_info();
  ebbrt::clock::SleepMilli(1*1000); // XXX(stephentu): give time for debug dump to reach console
...
mp_stress_test_allocator()
{
  ...
  for (auto &p : v)
    p->start();
  ebbrt::clock::SleepMilli(60*1000);
  running.store(false);
  for (auto &p : v) {
    p->join();
...
mp_stress_test_insert_removes()
{
  ...
  for (auto &p : v)
    p->start();
  ebbrt::clock::SleepMilli(5*1000); // let many epochs pass
  running.store(false);
  for (auto &p : v)
    p->join();
...
mp_test2()
{
  ...
  for (size_t txn_flags_idx = 0;
    ...
    ebbrt::clock::SleepMilli(30*1000);
    running = false;
    __sync_synchronize();
    w0.join();
    //w1.join();
...
mp_test_simple_write_skew()
{
  ...
  for (size_t txn_flags_idx = 0;
    ...
    for (size_t i = 0; i < NDoctors; i++)
      workers[i]->start();
    ebbrt::clock::SleepMilli(10*1000);
    running = false;
    __sync_synchronize();
