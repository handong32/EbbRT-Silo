...
public:
  ...
  core_id()
  {
    if (unlikely(tl_core_id == -1)) {
      ...
      //ebbrt::kprintf_force("In %s tl_core_id = %d\n", __FUNCTION__, tl_core_id);
  ...
  set_core_id(unsigned cid)
  {
    ...
    ebbrt::kprintf_force("cid = %d\n", cid);
    ebbrt::kprintf_force("NMaxCores = %d\n", NMaxCores);
    ebbrt::kprintf_force("g_core_count = %d\n", g_core_count.load(std::memory_order_acquire));
    ebbrt::kprintf_force("tl_core_id = %d\n", tl_core_id);*/
