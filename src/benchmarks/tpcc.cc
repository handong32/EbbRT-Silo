  ...
  try {
    ...
    if (strncmp(v_c.c_credit.data(), "BC", 2) == 0) {
      ...
      int n = snprintf(buf, sizeof(buf), "%d %d %d %d %d %d | %s",
                       ...
                       districtID,
                       warehouse_id,
                       static_cast<int>(paymentAmount),
                       v_c.c_data.c_str());
      /*int n = snprintf(buf, sizeof(buf), "%d %d %d %d %d | %s",
                       k_c.c_id,
                       k_c.c_d_id,
                       k_c.c_w_id,
                       districtID,
...
protected:
  ...
  make_loaders()
  {
    //KPRINTF("%s start - enable parallel loading:%d\n", __PRETTY_FUNCTION__, enable_parallel_loading);
  ...
  make_workers()
  {
    ...
    //KPRINTF("%s alignment=%d blockstart=%d\n", __FUNCTION__, alignment, blockstart);
...
tpcc_do_test(abstract_db *db)
{
  ...
  if (did_spec_remote_pct && g_disable_xpartition_txn) {
    KPRINTF("WARNING: --new-order-remote-item-pct given with --disable-cross-partition-transactions\n");
    KPRINTF("  --new-order-remote-item-pct will have no effect\n");
  }

  if (verbose) {
    KPRINTF("tpcc settings:");
    KPRINTF("  cross_partition_transactions : %d\n", !g_disable_xpartition_txn);
    KPRINTF("  read_only_snapshots          : %d\n", !g_disable_read_only_scans);
    KPRINTF("  partition_locks              : %d\n", g_enable_partition_locks);
    KPRINTF("  separate_tree_per_partition  : %d\n", g_enable_separate_tree_per_partition);
    KPRINTF("  new_order_remote_item_pct    : %d\n", g_new_order_remote_item_pct);
    KPRINTF("  new_order_fast_id_gen        : %d\n", g_new_order_fast_id_gen);
    KPRINTF("  uniform_item_dist            : %d\n", g_uniform_item_dist);
    KPRINTF("  order_status_scan_hack       : %d\n", g_order_status_scan_hack);
